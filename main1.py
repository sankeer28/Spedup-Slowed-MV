...
def main():
    ...
    video_id = input("Enter the YouTube video ID: ")  # Only enter the video ID, not the full URL
    ...
    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
        ...
        if video_title is None:
            ...
            return
        if speed_choice == '1':
            video_title = "slowed_down_" + re.sub(r'[\\/:*?"<>|]', '', video_title.replace(" ", "_"))
        else:
            video_title = "nightcore_" + re.sub(r'[\\/:*?"<>|]', '', video_title.replace(" ", "_"))
    ...
    download_video(f'https://www.youtube.com/watch?v={video_id}', video_filename)
