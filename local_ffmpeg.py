...
import requests
import re
# make sure to do git clone https://git.ffmpeg.org/ffmpeg.git ffmpeg
...
def extract_audio(video_file, output_audio, pitch=1.5, slow_down=False):
    if slow_down:
        pitch = 0.9
    command = f'{FFMPEG_PATH} -i "{video_file}" -vn -af "asetrate=44100*{pitch},aresample=44100" -acodec libmp3lame "{output_audio}"'
    os.system(command)
...
def combine_video_audio_image(image_file, audio_file, output_video, video_title, api_choice):
    try:
        ...
        resize_command = f'{FFMPEG_PATH} -i "{image_file}" -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" "{resized_image_file}" -y'
        os.system(resize_command)
        text = re.sub(r'[\\/:*?"<>|]', '', video_title.replace("_", " ").replace("nightcore", "").strip())
        text_command = f'{FFMPEG_PATH} -loop 1 -i "{resized_image_file}" -vf "drawtext=text=\'{text}\':x=w-tw-10:y=h-th-10:fontsize=40:fontcolor=black:shadowcolor=white:shadowx=2:shadowy=2" -r 30 "{image_file}_with_text.jpg"'
        os.system(text_command)
        command = f'{FFMPEG_PATH} -loop 1 -i "{image_file}_with_text.jpg" -i "{audio_file}" -c:v libx264 -c:a aac -strict experimental -b:a 192k -shortest "{output_video}"'
        os.system(command)
